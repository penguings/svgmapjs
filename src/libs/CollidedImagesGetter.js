class CollidedImagesGetter {
	...
	isCollided1(POIelemId) { // こちらはもっと以前から使われていないが、アルゴリズム的に有用かもしれないので残置(たぶんGISに移動するのが良い) 2018.2.1
	...
	isCollided(POIelemId) { // この機能はtestClickのcheckTickerへの吸収使われなくなったが、有用かもしれないので残置 2018.2.1
		...
		try { // 2016.6.15 isCollided関数がからの状態のoverwrappedPOIsを返すことがある？
			...
			for (var i in this.#visiblePOIs) {
				if (i != POIelemId) {
					if (targetPOI.x + targetPOI.width < this.#visiblePOIs[i].x ||
