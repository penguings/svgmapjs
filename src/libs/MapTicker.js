...
class MapTicker {
	...
	constructor(svgMapObject, matUtil, isEditingLayer, getLayerName, setLoadCompleted, svgMapAuthoringTool) {
		...
		if (!this.#ticker) {
			...
			this.#ticker.addEventListener("wheel", UtilFuncs.MouseWheelListenerFunc, false);
			this.#ticker.addEventListener("mousewheel", UtilFuncs.MouseWheelListenerFunc, false);
			this.#ticker.addEventListener("DOMMouseScroll", UtilFuncs.MouseWheelListenerFunc, false);
		...
		this.pathHitTester = new PathHitTester(svgMapObject, svgMapAuthoringTool, setLoadCompleted);
		this.poiHitTester = new PoiHitTester();
		this.#customHitTester = new CustomHitTester(svgMapObject, getLayerName);
		this.showPoiProperty = new ShowPoiProperty(svgMapObject, getLayerName, matUtil);
	...
	checkTicker(px, py) {
		...
		if (px && py) {
			...
			hittedLayerHitTests = this.#customHitTester.getLayerHitTestAtPoint(px, py); // 2022/05
		} else {
			...
			var mapCanvasSize = this.#svgMapObject.getMapCanvasSize();
			hittedPoiObjects = this.poiHitTester.getPoiObjectsAtPoint(mapCanvasSize.width / 2, mapCanvasSize.height / 2); // ラスタPOIでヒットしたモノ
			hittedLayerHitTests = this.#customHitTester.getLayerHitTestAtPoint(mapCanvasSize.width / 2, mapCanvasSize.height / 2, true); // 2022/05 , 2022/09 中心ヒットテスト判別可能にする
		}

		if (hittedPoiObjects.length == 0 && px && py && this.#checkAndKickEditor(hittedObjects, px, py)) { // POIがヒットしていない場合に限り、ベクタを対象にオーサリングツールのキック可能性をチェックし、キックされたならそのまま終了する
			return;
		}
		...
		if ((hittedObjects && hittedObjects.elements.length > 0) || hittedPoiObjects.length > 0 || hittedLayerHitTests.length > 0) {
			...
			var that = this;
			setTimeout(function () { that.#fixTickerSize() }.bind(this), 300);
			// for raster POI
			...
			if (hittedObjects) {
				for (var i = 0; i < hittedObjects.elements.length; i++) {
					var vMeta = this.showPoiProperty.getVectorMetadata(hittedObjects.elements[i], hittedObjects.parents[i], hittedObjects.bboxes[i]);
					var meta = this.showPoiProperty.getMetadataObject(vMeta.metadata, vMeta.metaSchema, vMeta.title);
					...
					}(hittedObjects.elements[i], hittedObjects.parents[i], hittedObjects.bboxes[i], this.showPoiProperty);
					lastCallback = vcbf;
					this.#addTickerItem(meta.title, vcbf, this.#tickerTable, vMeta.layerName);
					this.#tickerTableMetadata.push(
						{
							title: meta.title,
			...
			for (var i = 0; i < hittedLayerHitTests.length; i++) {
				...
				this.#addTickerItem(hitObj.title, cbf, this.#tickerTable, hitObj.layerName);
				this.#tickerTableMetadata.push(hitObj);
			}

			if (px && py && this.#tickerTableMetadata.length == 1) { // クリックモードで候補が一つだったら直接コールバック呼び出して、ティッカーは出現させない
	...
	#poiSelectProcess(obj) { // html:img要素によるPOI(from use要素)を１個だけの選択まで決定したあとに実行するプロセス
		...
		if (typeof this.#svgMapAuthoringTool == "object" && (el && el.getAttribute("iid") == this.#svgImagesProps[target.parentNode.getAttribute("id")].rootLayer)) { // 選択したオブジェクトが編集中レイヤのものの場合 (2019/3/12、タイルではなくレイヤーで判別するように変更)
	...
	#checkAndKickEditor(hittedVectorObjects, x, y) {
		var el = this.#isEditingLayer();
		var ans = false;
		if (typeof this.#svgMapAuthoringTool == "object" && el) { // オーサリングシステムがあり、オーサリング中のレイヤがある場合
	...
	#getEditingObject(hittedObjects, editingLayer) {
		...
		if (typeof this.#svgMapAuthoringTool == "object" && editingLayer) {
			for (var i = 0; i < hittedObjects.elements.length; i++) {
				if (editingLayer.getAttribute("iid") == UtilFuncs.getDocumentId(hittedObjects.elements[i])) {
					editingTarget = i;
					break;
				}
			}
	...
	#processShowUse(svgTargetObj) {
		var svgTarget = svgTargetObj.element;
		if (UtilFuncs.getHyperLink(svgTarget) && !svgTarget.getAttribute("content")) { // アンカーが付いていて且つメタデータが無い場合
			this.showPage(UtilFuncs.getHyperLink(svgTarget));
		} else if (UtilFuncs.getHyperLink(svgTarget) && svgTarget.getAttribute("content")) { // アンカーもあってメタデータもある場合
