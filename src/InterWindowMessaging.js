...
class InterWindowMessaging{
	constructor(functionSet, targetWindow_or_itsGetter, responseReady){
		console.log("InterWindowMessaging:functionSet, targetWindow, responseReady",functionSet, targetWindow_or_itsGetter, responseReady);
	...
	#setMessageListener(){
		window.addEventListener("message", async function(event){
			...
			if ( !targetWin || event.source.location.pathname != targetWin.location.pathname ) return;
			console.log("InterWindowMessaging get message:",event.data, " srcWin:",event.source," srcPath:",event.source.location.pathname);
			var msg = JSON.parse(event.data);
			if ( msg.command ){
				if ( this.#functionSet_int[msg.command]){
					var ans =await this.#functionSet_int[msg.command](...msg.parameter);
					var resp = {
					...
					console.log("========\ncmd:",msg.command," is not exists within commandSet");
	...
	#retryMax = 100;
	#postMessagePromise(messageJson) { // postMessageに対して返却があるケースはこちらを使ってawaitする
		return new Promise( async function(okCallback, ngCallback) {
			...
			while ( this.#readyState == false || this.#messageCallbackObj != null ){
				...
				if ( rc > this.#retryMax ){
					ngCallback("postMessagePromise: Now waiting message. skip.  command:"+(JSON.parse(messageJson)).command);
					return;
				}
				await this.#sleep(5);
				++rc;
