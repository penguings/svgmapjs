...
describe("unittest for EssentialUIs",()=>{
    describe("essentialuisでDOM操作に関する試験",()=>{
        ...
        it("layerListのサイズを自動調整",()=>{
            ...
            expect(result.getAttribute("style")).toBe("left: 30px; top: 10px; width: 300px; height: 90%; position: absolute;");  // layerListのサイズを設定する関数
        });
    ...
    describe("Geo2SVGを複数回呼び出す試験", ()=>{
        ...
        beforeAll(()=>{
            essentialui = new EssentialUIs(
                ...
                {
                    SVG2Geo:jest.fn().mockReturnValue({lat:130, lng:40}),
                    Geo2SVG:jest.fn().mockReturnValueOnce({x:300, y:200})
        ...
        it("setGeoViewPort",()=>{
            ...
            expect(mock_mapViewerProps.setRootViewBox).toHaveBeenCalledWith(
                {"height": 400, "width": 533.3333333333334, "x": 133.33333333333331, "y": 600}
            );
        });
    });
});