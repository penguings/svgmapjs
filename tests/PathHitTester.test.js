...
describe("target PathHitTest class",()=>{
	describe("",()=>{
		let pathHitter;
		let mock_SvgmapObj, mock_getMapCansasSize, mock_refreshScreen, mock_isEditingGraphicsElement;
		
		beforeAll(()=>{
			mock_getMapCansasSize = jest.fn().mockReturnValue({width: 300, height: 800});
			mock_isEditingGraphicsElement = jest.fn().mockReturnValue(null)
			mock_refreshScreen = jest.fn().mockReturnValue({"isEditingGraphicsElement":mock_isEditingGraphicsElement, });
			// 本当はsvgmapObjクラスをモック化する方がよいのだがないため、手動で作成
			mock_SvgmapObj = {"getMapCanvasSize": mock_getMapCansasSize, "refreshScreen": mock_refreshScreen};
