import { ResourceLoadingObserver } from "../src/libs/ResourceLoadingObserver";
...
describe("unittest for ResourceLoadingObserver",()=>{
    describe.each(statusPattern)("$description pattern ",(pattern)=>{
		...
		let mock_mapViewerProps, mock_svgImagesProps, mock_svgImages, mock_refreshScreen, mock_viewBoxChanged;
		...
		beforeEach(()=>{
			...
			resourceloadingobserver = new ResourceLoadingObserver(mock_mapViewerProps, mock_svgImagesProps, mock_svgImages, mock_refreshScreen, mock_viewBoxChanged);
			resourceloadingobserver.init(mock_imgRenderer, mock_mapTicker, mock_geometryCapturer);
        ...
        it("loadingImgs object have empty.",()=>{
			...
			// load完了のためイベント発火する
			expect(zoomPanEventListener).toBeCalledTimes(pattern.countFireTheZoomPanEvent);
			expect(screenRefreshedEventListener).toBeCalledTimes(pattern.countFireTheScreenFreshedEvent);
        });
