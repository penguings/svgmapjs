	...
	describe.each(flagPatterns)("target $description", (flagPattern) => {
		...
		beforeAll(() => {
			mock_geometryCapturer = {
				GISgeometriesCaptureOptions: { TreatRectAsPolygonFlag: true },
			};
		...
		beforeEach(() => {
			...
			pathrenderer.setSVGpathPoints = jest
				.fn()
				.mockReturnValue({ x: 10, y: 5, height: 7 });
			//
		...
		it("setSVGcirclePoints", () => {
			...
			let result = pathrenderer.setSVGcirclePoints(
				shapeNode,
				canvas,
				null,
				flagPattern.clickable,
				SvgMapElementType.CIRCLE,
				flagPattern,
				null
			);
		...
		it("setSVGrectPoints", () => {
			let canvas = document.createElement("canvas");
			let result = pathrenderer.setSVGrectPoints(
				shapeNode,
				canvas,
				null,
				flagPattern.clickable,
		...
		it("setSVGpolyPoints", () => {
			...
			let canvas = document.createElement("canvas");
			let result = pathrenderer.setSVGpolyPoints(
				shapeNode,
				canvas,
				null,
				flagPattern.clickable,
				SvgMapElementType.POLYGON,
				flagPattern,
				null
			);
