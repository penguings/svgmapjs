...
class ResumeManager {
	...
	#setCookie(key, value, expire) {
		...
		var rootPath = new URL(this.#svgImagesProps["root"].Path, location.href)
			.pathname;
		window.localStorage[
			ResumeManager.localStorageSvgMapSuffix + rootPath + "#" + key
		] = value;
	}
	...
	#getCookies() {
		...
		for (var i = 0; i < window.localStorage.length; i++) {
			if (window.localStorage.key(i).indexOf(lss) == 0) {
				result[window.localStorage.key(i).substring(lss.length)] =
		...
		if (initialCustomLayersObj.customLayersSettings) {
			...
			if (!initialCustomLayersObj.currentSettingKey) {
				if (initialCustomLayersObj.currentSettingKeys?.customLayer) {
					initialCustomLayersObj.currentSettingKey =
						initialCustomLayersObj.currentSettingKeys.customLayer;
				} else {
					// 適当に選んでしまう・・・
			...
			if (initialCustomLayersObj.metadata) {
				if (initialCustomLayersObj.metadata.viewBox) {
					this.#initialCustomLayers.viewBox =
						initialCustomLayersObj.metadata.viewBox;
				}
				if (initialCustomLayersObj.metadata.settingRevision) {
					this.#initialCustomLayers.settingRevision =
						initialCustomLayersObj.metadata.settingRevision;
				}
				if (initialCustomLayersObj.metadata.rootContainerHref) {
					this.#initialCustomLayers.rootContainerHref =
						initialCustomLayersObj.metadata.rootContainerHref;
				}
			}
			this.#initialCustomLayers.customLayersSettings.startup.metadata.key =
				"startup";
		}
	}
	...
	checkResume(documentElement, symbols) {
		...
		if (this.resumeFirstTime) {
			...
			if (this.#initialCustomLayers && this.#svgMapCustomLayersManager) {
				// 2024/08/06 initialCustomLayersの処理を行う
				try {
					console.log(
			...
			if (initialCustomViewBox) {
				this.#svgMapObject.setGeoViewPort(
					initialCustomViewBox.y,
				...
				if (customGeoViewboxes.currentSettingKey) {
					var cvb =
						customGeoViewboxes.settings[customGeoViewboxes.currentSettingKey];
					if (cvb) {
						this.#svgMapObject.setGeoViewPort(
							cvb.y,
							cvb.x,
		...
		if (this.resumeFirstTime) {
			...
			if (lh) {
				...
				if (lh.hiddenLayer) {
					if (lh.hiddenLayer.indexOf("*") >= 0) {
						// ワイルドカード*が設定されていたら、全レイヤーをhideする　その他のURLは無視する。#オプション部も無視されてしまうが・・
	...
	setResume(stat) {
		...
		this.#resume = stat;
		if (!this.#resume || Object.keys(this.#svgImages).length > 0) {
			// 2017.8.22 debug 2017.9.29 dbg2: onload直後でsetResume(true)するとエラーアウトしてresumeできない : 2017.10.03 dbg3: svgImagesPropsは作られていてもsvgImagesがない場合エラーするので・・Object.keys(svgImagesProps).length -> Object.keys(svgImages).length に
			this.#saveResumeData();
		}
	}
