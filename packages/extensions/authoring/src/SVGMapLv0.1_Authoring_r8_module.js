...
class SvgMapAuthoringTool {
	...
	#initPOIregistTool(
		...
		if (
			...
			this.#uiMapping.editingMode == "POIreg" &&
			uiDoc === this.#uiMapping.uiDoc &&
			poiDocId == this.#uiMapping.editingLayerId
		) {
	...
	#editConfPhase2(targetDoc, toolsCbFunc, toolsCbFuncParam, confStat) {
		...
		if (toolsCbFunc) {
			var retVal;
			if (this.#uiMapping.returnSvgElement) {
				// 2020/7/17
				var attrs = null;
	...
	#setPolySvg(targetDoc, poiDocId) {
		...
		if (
			geoPoints.length < 2 ||
			(this.#uiMapping.editingMode == "POLYGON" && geoPoints.length < 3)
		) {
			return false;
		}
		...
		if (
			this.#uiMapping.modifyTargetElement &&
			(this.#uiMapping.modifyTargetElement.nodeName == "polygon" ||
				this.#uiMapping.modifyTargetElement.nodeName == "polyline")
		) {
	...
	#setPoiRegPosition(e, targetTxtBoxId, directPutPoiParams) {
		...
		targetDoc.getElementById(targetTxtBoxId).value =
			...
			"," +
			this.#svgMap.numberFormat(geop.lng);
	...
	#callAfterRefreshed(cbf, cbfParam0, cbfParam1) {
		...
		var updateCanvas = function () {
			...
			if (this.#uiMapping.insertPointsIndex >= 0) {
				if (
					...
					this.#uiMapping.editingMode == "POLYLINE" &&
					this.#uiMapping.insertPointsIndex == 0
				) {
					hilightPoint(0);
				} else {
					if (this.#uiMapping.insertPointsIndex == 0) {
	...
	#addPoiEditEvents(targetCanvasNode) {
		// 不使用
	...
	#setTargetObject(svgTarget) {
		...
		if (
			this.#uiMapping.editingLayerId === svgTarget.docId ||
			this.#uiMapping.editingLayerId ===
				this.#svgImagesProps[svgTarget.docId].rootLayer
		) {
			// 冗長・・
	...
	#hilightPOI(poiID) {
		...
		if (
			...
			document.getElementById(this.#selectedObjectID)
		) {
	...
	#displayPOIprops(svgTarget) {
		...
		pep.value =
			this.#svgMap.numberFormat(props.position.lat) +
			"," +
			this.#svgMap.numberFormat(props.position.lng);
	...
	#updatePointListForm(pep, points) {
		...
		for (var i = 0; i < points.length; i++) {
			taVal +=
				...
				this.#svgMap.numberFormat(points[i].lat) +
				", " +
				this.#svgMap.numberFormat(points[i].lng) +
				'"/></td></tr>';
		}
	...
	#editPolyPoint = function (e) {
		...
		if (
			this.#prevMouseXY.x == mxy.x &&
			this.#prevMouseXY.y == mxy.y &&
			this.#pointAddMode == false
		) {
			//		document.removeEventListener("click", arguments.callee, false);
			this.#removePointEvents(this.#editPolyPoint);
		...
		targetDoc.getElementById("polyEditor").addEventListener(
			...
			function (e) {
				...
				if (e.target.id.indexOf("point") == 0) {
					...
					if (
						this.#uiMapping.selectedPointsIndex >= 0 ||
						this.#uiMapping.insertPointsIndex >= 0
					) {
	...
	#getMetaSchema(docId) {
		...
		var metaSchema = null;
		if (
			this.#svgImages[docId].documentElement.getAttribute("property") &&
			this.#svgImages[docId].documentElement.getAttribute("property").length > 0
		) {
			metaSchema = this.#svgImages[docId].documentElement
	...
	// var toolsCbFunc; // uiMapping.toolsCbFuncに収納変更
	// var toolsCbFuncParam; // 同上
	#initFreeHandTool(targetDiv, poiDocId, cbFunc, cbFuncParam, options) {
	...
	#setFreeHandEvent(targetDoc, poiDocId) {
		...
		targetDoc.getElementById("freeHandAdd").addEventListener(
			"click",
			function (e) {
	...
	#initGenericTool(targetDiv, poiDocId, cbFunc, cbFuncParam, options) {
		this.#removeChildren(targetDiv);
		var uiDoc = targetDiv.ownerDocument;
	...
	**/
		...
		var modeSelDivHTML = `	<input type="radio" value="poi" id="${
			this.#pointToolRadio
		}" name="amode" checked></input><label for="${
			this.#pointToolRadio
		}">POINT</label>
<input type="radio" value="polyline" id="${
			this.#polylineToolRadio
		}" name="amode" ></input><label for="${
			this.#polylineToolRadio
		}">POLYLINE</label>
<input type="radio" value="polygon" id="${
			this.#polygonToolRadio
		}" name="amode" ></input><label for="${
			this.#polygonToolRadio
		}">POLYGON</label>`;
		if (options?.withBufferedTools) {
			this.#uiMapping.genericMode.withBufferedTools = true;
			modeSelDivHTML += `	<input type="radio" value="b_poi" id="${
				this.#bufferedPointToolRadio
			}" name="amode" ></input><label for="${
				this.#bufferedPointToolRadio
			}">CIRCLE</label>
<input type="radio" value="b_polyline" id="${
				this.#bufferedPolylineToolRadio
			}" name="amode" ></input><label for="${
				this.#bufferedPolylineToolRadio
			}">Buffered POLYLINE</label>
<input type="radio" value="b_polygon" id="${
				this.#bufferedPolygonToolRadio
			}" name="amode" ></input><label for="${
				this.#bufferedPolygonToolRadio
			}">Buffered POLYGON</label>`;
		}

		modeSelDiv.insertAdjacentHTML("beforeend", modeSelDivHTML);

		uiDoc
			.getElementById(this.#pointToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);
		uiDoc
			.getElementById(this.#polylineToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);
		uiDoc
			.getElementById(this.#polygonToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);

		uiDoc
			.getElementById(this.#bufferedPointToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);
		uiDoc
			.getElementById(this.#bufferedPolylineToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);
		uiDoc
			.getElementById(this.#bufferedPolygonToolRadio)
			.addEventListener("change", this.#changeGenericToolMode);
	...
	#clearChangeGenericToolMode(uiDoc) {
		uiDoc
			.getElementById(this.#pointToolRadio)
			.removeEventListener("change", this.#changeGenericToolMode, false);
		uiDoc
			.getElementById(this.#polylineToolRadio)
			.removeEventListener("change", this.#changeGenericToolMode, false);
		uiDoc
	...
	#switchGenericTool(svgTarget) {
		...
		if (this.#uiMapping.bufferOption) {
			currentEditingMode_w_bufferMode = "B_" + currentEditingMode_w_bufferMode;
		}
		...
		if (
			buffered &&
			this.#uiMapping.uiDoc.getElementById("objectBufferLength")
		) {
			this.#uiMapping.uiDoc
	...
	cancelPointingPoiRegister(...params) {
		return this.#cancelPointingPoiRegister(...params);
	}
	editPoint(...params) {
		return this.#editPoint(...params);
	}
	initFreeHandTool(...params) {
		return this.#initFreeHandTool(...params);
	}
	initGenericTool(...params) {
		return this.#initGenericTool(...params);
	}
	initPOItools(...params) {
		return this.#initPOItools(...params);
	}
	initPOIregistTool(...params) {
		return this.#initPOIregistTool(...params);
	}
	initPolygonTools(...params) {
		return this.#initPolygonTools(...params);
	}
	setTargetObject(...params) {
		return this.#setTargetObject(...params);
	}
	isEditingGraphicsElement(...params) {
		return this.#isEditingGraphicsElement(...params);
	}
	clearTools(...params) {
		return this.#clearTools_with_UI(...params);
	}
}
